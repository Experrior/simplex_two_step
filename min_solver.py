...
def solve(bound):
    """
        Main function for solving linear minimization problem.
    ...
    if round(simplex_tableau[-1][-1], 4) != 0:
        ...
        for iterator in to_delete:
            for i in range(len(solutions)):
                if solutions[i] >= to_delete[iterator]:
                    solutions[i] += 1
            for i in range(len(basic_variables)):
                if basic_variables[i] != -1 and basic_variables[i] >= to_delete[iterator]:
                    basic_variables[i] += 1
        ...
        if message == "finished" and len(solutions) != 0:
